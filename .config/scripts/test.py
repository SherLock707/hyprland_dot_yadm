...
def get_icon_path_from_desktop_file(path, size=48):
...
def get_icon_for_app(app_name):
    ...
    substituted_name = CLASS_SUBSTITUTIONS.get(app_name, app_name)
    
    desktop = find_desktop_file(substituted_name)
    if desktop:
        return get_icon_path_from_desktop_file(desktop)
    
    # If substitution didn't work, try original name
    if substituted_name != app_name:
        ...
        if window_height > 60:
            ...
            if icon_path and os.path.exists(icon_path):
                try:
                    ...
                    icon_container.add(icon_image)
                except Exception as e:
                    print(f"Error loading icon {icon_path}: {e}")
            
            window_content.add(icon_container)
