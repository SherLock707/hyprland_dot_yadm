 ...
 * Usage: ./rain [--rain-color COLOR] [--lightning-color COLOR] [--intensity LEVEL] [--puddles] [--flash]
 * Press 't' for thunderstorm, 'w' for wind, '+/-' for intensity, 'p' for puddles, 'h' for help
 */
...
static void simulate_rain(const char *rain_color, const char *lightning_color) {
    ...
    while (1) {
        ...
        if (key == KEY_RESIZE) {
            ...
            clear();
            mvprintw(rows/2 - 5, cols/2 - 20, "=== RAIN SIMULATOR CONTROLS ===");
            mvprintw(rows/2 - 3, cols/2 - 20, "T - Toggle thunderstorm");
            mvprintw(rows/2 - 2, cols/2 - 20, "W - Toggle wind");
            mvprintw(rows/2 - 1, cols/2 - 20, "P - Toggle puddles");
            mvprintw(rows/2, cols/2 - 20, "F - Toggle flash effect");
        ...
        for (int i = 0; i < raindrop_count; i++) {
            ...
            // Create puddle when raindrop hits ground
        ...
        // Draw stats
        if (show_stats) {
            mvprintw(0, 0, "Drops:%d Bolts:%d Puddles:%d | Wind:%.1f Int:%.1fx | Puddles:%s Flash:%s | [H]elp", 
                     raindrop_count, bolt_count, puddle_count, wind_strength, intensity_multiplier,
                     puddles_enabled ? "ON" : "OFF", flash_enabled ? "ON" : "OFF");
        }
